...
- [Transcrypt Product Requirements Document (PRD)](#transcrypt-product-requirements-document-prd)
  ...
  - [6. Compliance and Governance Framework](#6-compliance-and-governance-framework)
    ...
    - [6.5 Legislative Alignment and Future Adaptation](#65-legislative-alignment-and-future-adaptation)
    - [6.6 Governance of Product Lifecycle](#66-governance-of-product-lifecycle)
  - [7. Security and Infrastructure Requirements](#7-security-and-infrastructure-requirements)
    ...
    - [Key Management Model](#key-management-model)
...
Transcrypt’s own credibility rests on the same standards it helps others to meet. Its internal compliance and governance framework is therefore designed as a living framework of assurance that mirrors the principles of the product itself: transparency, proportionality, and continuous verification rather than static certification. The platform is engineered and operated within a structure that satisfies legal, ethical, and regulatory obligations from day one but remains light enough for a founder-led company. Governance is treated as architecture, not paperwork — embedded in code, workflows, and data handling policies.

### 6.1 Alignment with NIS, Cyber Essentials, ISO 27001, and IEC 62443

Transcrypt’s governance and assurance model is anchored in alignment with NIS, Cyber Essentials, ISO 27001, and IEC 62443—the four frameworks that together define the contemporary cybersecurity landscape across IT and operational domains. From NIS, Transcrypt inherits the organising principle of proportionate resilience: that security controls must scale with the service’s criticality and threat exposure. The platform’s architecture enforces this by separating control logic, customer data, and AI functions into isolated components, ensuring that failure or compromise in one cannot propagate to another. Its logging, monitoring, and recovery workflows map directly to NIS obligations for availability, integrity, and incident management. In practice, the NIS alignment ensures that Transcrypt can one day act not only as a compliance platform for SMEs but also as a trusted node in the wider regulated ecosystem, capable of producing audit-grade evidence recognised by operators of essential services.

Cyber Essentials defines Transcrypt’s day-to-day operational hygiene. The five technical control themes—patching, secure configuration, access control, malware protection, and boundary defence—are embedded directly into the platform’s development and hosting pipelines. Every build undergoes automated vulnerability scanning, dependency patching, and configuration validation before deployment. Access to production systems is limited to MFA-enforced identities on managed devices, and malware prevention is treated as a base condition for participation in the network. This mapping is not theoretical: it underpins Transcrypt’s own certification plan and gives the platform credibility with small businesses who must demonstrate these same controls. The detailed automation, scope, and monetisation model for Cyber Essentials v3.2 lives in [§6.2 Cyber Essentials Alignment (v3.2)](#62-cyber-essentials-alignment-v32), ensuring a single canonical reference for implementation specifics. The outcome is a product that practises what it preaches—internally compliant with the same framework it helps its customers to achieve.

At the governance layer, ISO 27001 and IEC 62443 together define Transcrypt’s structural DNA. ISO 27001 provides the managerial skeleton—risk management, control review, continual improvement—while IEC 62443 adds the engineering muscle: secure development lifecycle, least-privilege zoning, and explicit separation of safety and control functions. These combined standards shape both the software’s architecture and its operational ethos. Every significant change follows an assess-design-verify-release cycle derived from 62443-4-1, and the network model reflects the zone-and-conduit logic of 62443-3-3. This dual alignment signals that Transcrypt is not merely an IT compliance platform but an automation-aware security product—built to satisfy the same expectations that govern critical infrastructure. By grounding its internal governance in these frameworks, Transcrypt gains not only regulatory credibility but also a blueprint for resilience that will scale with its customers’ complexity.

### 6.2 Cyber Essentials Alignment (v3.2)

**Purpose**
This section defines how Transcrypt implements and monetises Cyber Essentials v3.2 (CE) for UK SMEs.
It establishes the mapping between the NCSC’s five technical control families and Transcrypt’s product modules, ensuring that every customer action within the platform directly advances certification readiness.

**Control Families → Product Mapping**

| Cyber Essentials Control | Platform Module | Key Automation Outcome | Commercial Hook |
|:--------------------------|:----------------|:-----------------------|:----------------|
| **1. Firewalls** | Network Baseline Wizard | Auto-discovers gateways, checks inbound/outbound rule compliance | Tier-1 report credit per validated gateway |
| **2. Secure Configuration** | Device Policy Library | Validates OS/app configurations against Cyber Essentials v3.2 baselines | Upsell: Managed Policy Pack subscription |
| **3. Security Update Management** | Patch Tracker + [Evidence Vault](#325-evidence-services) | Flags missing high/critical updates (>CVSS 7) beyond 14 days; logs proof | Renewal incentive: “auto-renew with live patch telemetry” |
| **4. User Access Control** | Identity and MFA Scanner | Enumerates privileged accounts; checks MFA and password policies | Add-on: “Admin-Account Audit” mini-service |
| **5. Malware Protection** | Endpoint and Cloud Coverage Dashboard | Confirms AV/allow-listing and signature update status | Cyber Essentials Plus–ready evidence report |

Operational enforcement for each mapping will be delivered by the [Rule Evaluation Service](#323-rule-evaluation-service-deterministic), the [Evidence Services](#325-evidence-services), and the reporting stack outlined in [§5.3](#53-reporting-dashboards-and-insights-v1), ensuring that data capture, automated testing, and monetisation hooks stay tightly coupled.

**Scope Engine Integration**
Transcrypt’s scoping workflow will mirror Cyber Essentials v3.2 Section C, incorporating:
- Inclusion of all devices accessing organisational data (corporate or BYOD).
- Automatic detection of cloud, SaaS, and remote-work assets under the shared-responsibility model.
- Exclusion logic for clearly defined “sub-sets,” with audit-trail justification stored in `scope.manifest.json`.
These rules are enforced by the intake-to-evaluation pathway documented in [§5.1 Data Flow Architecture (v1)](#51-data-flow-architecture-v1), ensuring there is no divergence between scope definition and execution.

**Canonical Data Model Terms**  
To maintain interoperability with NIS 2 and ISO 27001 mappings, Transcrypt adopts the official Cyber Essentials v3.2 taxonomy:
*device, software, organisational data, organisational service, account, controlset, evidenceitem.*

**Evidence and Reporting Alignment**
Each control family will emit structured Evidence Items corresponding to the IASME question set (v15.1).
- Evidence files will be hashed and timestamped.
- Evidence payloads will include a mandatory `ce_ref` field (for example, `CE-3.2-UAC-Q17`) so that question references flow through ingestion, dashboards, and exports without manual tagging.
- One-click Self-Assessment export (CSV or JSON) will follow the IASME booklet ordering, allowing customers to submit straight from Transcrypt without reformatting.
- Renewal reports will auto-generate when any control shows “stale > 90 days.”
All exports will originate from the [Report Service](#326-report-service) and [Compliance reporting & dashboards](#53-reporting-dashboards-and-insights-v1) capabilities so that evidence, scoring, and submissions remain synchronised.

**Control Enforcement Thresholds**
The Cyber Essentials v3.2 defaults will be codified in deterministic rules so that automation outcomes are evidence-ready and auditor-recognisable.
- Critical and high-risk vulnerabilities must be remediated within 14 days; Patch Tracker will escalate any breach of this threshold to the renewal workflow.
- MFA will be required for all cloud service authentication, and administrative accounts must enforce MFA before access is granted.
- Password policy will enforce at least 12 characters by default, or a minimum of 8 characters when paired with a deny-list; forced rotation policies will be rejected unless justified in the scope manifest.
- Device unlock policies must throttle or lock out access after 10 failed attempts or fewer, with enforcement status surfaced inside the Device Policy Library.
- Only licensed and supported software will remain in scope; unsupported packages will trigger exception handling and automatic evidence updates.

**RulePack Versioning and SLA**
Transcrypt will publish Cyber Essentials automation as `RulePack-CE-v3.2.Y`, committing to ship updates within 30 days of any official change. Each pack will carry semantic versioning metadata inside the ControlSet schema so that downstream mappings (including NIS 2 and ISO 27001) inherit compatibility guarantees.

**Strategic Value**  
- Transcrypt will monetise recurring compliance, not one-off audits.
- Aligning to Cyber Essentials v3.2 will ensure immediate product-market fit for 200 000+ UK-certified organisations and supply-chain mandates.
- Future releases (see §9 Roadmap) will extend the ControlSet schema to NIS 2, ISO 27001 Annex A, and IEC 62443 zones/conduits for multi-framework convergence.

**References**  
- NCSC Cyber Essentials Requirements for IT Infrastructure v3.2 (April 2025)  
- IASME Cyber Essentials Question Booklet v15.1 (March 2025)  
- NCSC Zero Trust Architecture Guidance (2024)
...
Together, the Policy Library and Evidence Management subsystems close the loop between regulation, implementation, and proof. The former provides the structured definitions of what must be true; the latter provides the demonstrable artefacts that it is true. This alignment enables Transcrypt’s internal operations to be as auditable as the platform it offers to others. It also creates a self-reinforcing trust model: every control the company enforces on itself strengthens the logic it delivers to customers. By embedding compliance logic and evidence within the same unified data fabric, Transcrypt ensures that assurance is not a bureaucratic overlay but a living, queryable framework—transparent to regulators, customers, and itself.

### 6.5 Legislative Alignment and Future Adaptation
...
### 6.6 Governance of Product Lifecycle
...
Incident response is governed by a structured, time-bound playbook model. Every alert or anomaly automatically creates an incident record containing the event context, platform state snapshot, and relevant logs. Severity classification triggers predefined escalation paths—technical triage, containment, eradication, and recovery—all tracked in a shared operations log. The first responder focuses on evidence preservation and containment; post-incident, a root cause analysis (RCA) is generated, peer-reviewed, and committed to version control as a permanent artefact. This ensures that lessons learned are actionable, repeatable, and auditable. Each incident closes with a measurable improvement—either a control hardening, a detection enhancement, or an updated runbook—so that the platform continually evolves from experience.
