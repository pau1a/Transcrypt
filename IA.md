...
This section sets the rules that govern every internal cross-link on the site. All lateral links (related content, topic pivots, industry suggestions, SEO clusters) must be produced only by the partials listed in Appendix D. No page may add unmanaged or ad-hoc cross-links.

## **9.0 Core rules (apply to everything)**

* All lateral cross-links must be generated by a canonical partial from Appendix D. Hardcoded lists or freeform inline related links are forbidden.
* Every content page must identify its `owner_id` and `taxonomy_ids` in metadata. Partials use those fields to choose targets.
* Marketing and Essentials runtimes are separated. Marketing partials are suppressed in Essentials unless the PDS explicitly permits a read-only variant.
* If a partial cannot find qualified, permissioned targets, it must hide itself. Never surface low-confidence or unauthorized links.

---

## **9.1 Mandatory Internal Links**

These links must appear on the page types indicated by Appendix B and be implemented via the named partials in Appendix D.

Rules (by page type)

* **Content pages (blogs, guides, long-form)**

  * Include `breadcrumbs` (partial) at the top.
  * Include `related-stories` (partial) at the end (3–5 items) or hide it if no qualified candidates.
  * Include `next-best-step` (partial) once in the CTA zone if the page has an actionable outcome.
  * Include `author-profile` (partial) on long articles.
* **Hub / topic index pages**

  * Include `topic-landing` or a hub composition of `for-your-industry`, `research-highlight`, and `conversion-cta`.
  * Provide `pagination` for lists and emit `rel=prev`/`rel=next`.
* **Resource / asset pages**

  * Include `resource-download` (or gated CTA) and `related-stories` (where relevant).
* **Essentials / product task pages**

  * Include `product-next-step` (partial) inside the workflow. Do not include marketing discovery partials.
  * Include `internal-support-rail` or `support-quicklink`.
* **All pages**

  * Include `legal-footer-cluster` in the footer (never suppressed on transactional pages).
  * Include `schema-inject` or equivalent head metadata (canonical, OpenGraph, JSON-LD) as required by SEO appendix.

Enforcement

* CMS must validate required partials per page type on save/publish and fail the publish if missing.
* Pre-publish CI job must verify `owner_id` and at least one `taxonomy_id` exist for content pages.

---

## **9.2 Contextual Relevance Links**

How partials choose which targets to show.

Selection rules (applies to `related-stories`, `for-your-industry`, `doc-related-guides`, etc.)

1. **Taxonomy first** — candidate must share at least one `taxonomy_id`. Prefer candidates with 2+ matches.
2. **Intent compatibility** — compare `seo_primary_intent` of source and candidate. Exclude or demote candidates whose primary intent conflicts (e.g., transactional vs informational).
3. **Editorial override** — if the page carries an editorially curated list, use that list instead of algorithmic picks. Editorial picks always win.
4. **Scoring** — score candidates with:
   `score = 0.6 * taxonomy_score + 0.25 * authority_score + 0.15 * recency_score`

   * Minimum taxonomy weight = 0.4.
   * If no candidate meets the minimum score threshold, hide the partial or show editorial fallback.
5. **Permission & tenancy** — for tenant-scoped content, verify user permissions before showing links; if unauthorized, show a gated CTA or hide the link.

Placement & density

* `related-stories` shows 3–5 items. `you-may-also-like` shows 2–4 lighter items. Do not exceed 8 lateral content links in the main content area.
* Inline glossary uses `glossary-inline` per term; do not use more than 2 inline glossary widgets in the article body unless editorially justified.

UX guidelines

* Label marketing tiles clearly when they are promotional or affiliate content.
* For product flows, use guidance partials (`product-next-step`) that lead to deterministic next actions, not discovery.

---

## **9.3 SEO-Driven Link Clusters**

Structure and rules that concentrate internal link equity and prevent keyword cannibalisation.

Cluster model

* **Owner page (canonical)** — single canonical page (`owner_id`) for each topic. This page is the primary target for internal links on the topic.
* **Hub / index pages** — aggregate supporting content and point to the owner.
* **Supporting pages** — guides, articles, case studies; they link *to* the owner and to relevant siblings.

Canonicalisation & sitemaps

* Owner pages must be declared in Appendix B and in the sitemap as cluster roots.
* Pagination must canonicalise page 1 to the base path and use `rel=prev/next` on paginated pages.
* Partials must supply canonical URLs from Appendix B; partials must not create custom permalink variants.

Linking behaviour for SEO

* Supporting pages should contain exactly one prominent internal link to the owner (use `next-best-step` or `related-stories` with owner pinned). Additional lateral links limited to 2–3 sibling pages.
* Avoid multiple pages in the same cluster competing for the same head term. If two pages vie for the same intent, update Appendix B owner mapping and adjust internal links rather than adding more links.

Monitoring & metrics

* Track: partial impressions and clicks (`partial.{id}.impression` / `partial.{id}.click`), CTR, bounce rate for pages surfaced by partials, and SERP rank movement for owner head terms.
* Daily automated report: clusters where a non-owner page receives >20% of internal link weight for the head term — flag for editorial cleanup.

---

## **9.4 Cross-Link Suppression Rules**

When partials must not appear or must change behaviour.

Hard suppression (never show these partials)

* Marketing discovery partials (`related-stories`, `for-your-industry`, `affiliate-block`, `subscribe-widget`) on Essentials runtime pages and on any transaction confirmation or payment entry screens.
* Any partial that surfaces indexable content on pages with `indexable=false`. Convert to gated CTA or hide.

Contextual suppression

* Suppressed shell states (reduced header/footer) must still surface legal/trust links (`legal-footer-cluster`) but must not show discovery or promotional partials.
* During sensitive multi-step workflows (intake, evidence edit, billing) hide discovery partials; only show deterministic workflow guidance (`product-next-step`) and support links.

Permission and tenancy suppression

* If a partial would return tenant-scoped targets, check tenancy and permissions before rendering. Unauthorized users see a gated CTA or no link.

Operational suppression

* If a partial’s algorithm cannot meet the minimum confidence threshold, hide the partial. Never display fallback low-quality links.

Enforcement and testing

* CMS validators reject placement of prohibited partials in templates or page instances.
* CI test suite includes:

  * render tests for suppressed templates to assert no marketing partials appear,
  * taxonomy validation tests (pages with partials must have required metadata),
  * permission tests for tenant-scoped partials.
* Audit logs record every partial render and click with `partial-id`, `page_id`, `user_role` and timestamp.

---

## **9.5 Implementation checklist (short)**

* Map each page type in Appendix B to its required and optional partial slots.
* Implement CMS validators to block prohibited partial placements and to require `owner_id` / `taxonomy_ids` where partials are used.
* Add CI jobs: partial input validation, suppressed-page render tests, cluster link-weight report.
* Wire analytics: standardized event names and attributes for partials (render + click).
* Maintain an editorial remediation workflow for cluster issues flagged by reports.

---

### Examples (copyable)

* On a guide page: include `breadcrumbs`, `next-best-step` (owner pinned), `related-stories` (3 items), and `author-profile`. If `related-stories` has no qualified items, hide it.
* On an evidence overview in Essentials: include `product-next-step`, `related-evidence-jump` (permissioned), `internal-support-rail`. Do not include `related-stories` or `subscribe-widget`.
* On a hub page: include `topic-landing` or a composition of `for-your-industry`, `research-highlight`, `conversion-cta`, plus `pagination` if needed.

---

# **10. Breadcrumb Rules**

Breadcrumbs orient users and show where a page sits in the site hierarchy. This section specifies exactly when breadcrumbs appear, how they are derived, how they render, how they behave responsively, how and when they are suppressed, and how they integrate with APIs, inference services, and monitoring. All rules below assume Appendix B (Page Inventory), Appendix C (Metadata Schema), Appendix D (Partials), and Appendix E (Taxonomy) are authoritative sources.
...
1. Show breadcrumbs only on these public/marketing and docs page types:

   * Guides, long-form articles, blog posts, hub/topic index pages, docs/API reference pages, resource/asset pages, report landing pages.
2. Do NOT show breadcrumbs on:

   * Homepage, single-page microsites, checkout/payment entry and confirmation, modal-only UIs, embedded widgets, tenant admin screens, and any page where `breadcrumbs_allowed = false` in metadata.
3. Essentials (product/runtime) exception:

   * Show breadcrumbs in Essentials only when they map to the public site hierarchy (i.e., breadcrumb leads to public parent pages). Do not expose tenant-internal task structure. Use workflow stepper (`product-next-step`) instead for internal flows.
4. Preconditions:

   * Eligible pages must have `page_id` and at least one of `owner_id` or `taxonomy_ids`. CMS must block breadcrumb rendering when those fields are absent.

---

## **10.2 Hierarchy Mapping Rules**

Derivation priority (deterministic)

1. Primary: `owner_id` → resolve to canonical parent chain from Appendix B.
2. Secondary: highest-priority `taxonomy_id` per Appendix E → resolve parent chain.
3. Tertiary: canonical path segments only if neither owner nor taxonomy present.
4. Multi-parent conflict resolution (deterministic tie-breaker):

   * Use `breadcrumb_priority` metadata if set;
   * Else use taxonomy priority (Appendix E weight);
   * Else older creation date wins;
   * Record chosen parent as `chosen_breadcrumb_parent` in page metadata at publish.
5. Inference & disambiguation:

   * If the mapping is ambiguous, call the internal inference service `/api/inference/resolve-breadcrumb` with inputs `{ page_id, title, taxonomy_ids }`. The service returns a ranked parent candidate list and confidence score. If confidence ≥ `0.8` use the top candidate and store it; if < `0.8` surface editorial override only (do not auto-pick).
6. Performance & determinism:

   * Breadcrumb resolution must be precomputed at publish time (CMS job) and cached. The page render path must not perform live network inference calls.
7. URLs:

   * Use canonical URLs from Appendix B for each breadcrumb item. Strip query parameters and fragments.
...
1. Visual structure:
   ...
   * `Home › Section › Subsection › Current Page` (current page is inert text, not a link).
   * Show up to 5 visible items by default; deeper chains collapse per 10.4.
2. Labels:

   * Use `breadcrumb_label` metadata if present. Fallback to page title truncated to 60 chars. Do not include parenthetical notes or internal IDs.
3. Accessibility:

   * Wrap breadcrumbs in `<nav aria-label="Breadcrumb">` and an ordered list `<ol>`. Each link must include accessible name, descriptive text, and logical focus order.
4. SEO:

   * Emit `BreadcrumbList` JSON-LD matching the visible trail and canonical URLs. Also ensure the page head contains the canonical `<link>` tag.
5. Gated/Restricted parent handling:

   * If a parent is gated/non-public, show parent as inert label (no link) and append an unlock hint where appropriate (e.g., “(requires login)”) or link to a public summary page if one exists.
6. UX micro-rules:

   * Do not show more than one breadcrumb instance per page.
   * Separator glyph: use `›` (plain ASCII equivalent). No decorative punctuation.
   * Each breadcrumb item must be short: prefer 1–6 words.

---

## **10.4 Responsive / Collapsed Breadcrumbs**

Mobile and overflow behaviour

1. Mobile default:

   * Show two items: `Home › … › Current` (ellipsis is interactive).
   * Tapping the ellipsis opens an accessible vertical list of the full breadcrumb chain.
2. Tablet / Desktop:

   * Show up to 4 items inline. If depth > 4 collapse middle items into an overflow menu (ellipsis). Overflow preserves canonical order.
3. Interaction details:

   * Overflow menu must be keyboard-navigable and screen-reader announced (e.g., “Breadcrumb expansion — list of ancestors”).
   * Expansion must not reflow the page in a way that displaces interactive elements unexpectedly.
4. Performance:

   * All breadcrumb rendering must be client-light. Use precomputed parent chains embedded in page HTML (from publish-time job). Do not resolve hierarchy client-side via network calls.
5. Visual density:

   * Ensure contrast and hit-target sizes meet WCAG AA at all breakpoints.

---

## **10.5 Suppressed Breadcrumb Cases**

When breadcrumbs must not render or must be altered

1. Hard suppression (no breadcrumbs):

   * Checkout/payment entry and confirmation; tenant admin consoles; any full-screen modal that suppresses global chrome; pages with `breadcrumbs_allowed = false`.
2. Conditional suppression or alteration:

   * If the user is mid-workflow in Essentials, replace site breadcrumbs with a workflow stepper (`product-next-step`) or show only top-level cluster label (not clickable).
   * If parent chain includes embargoed or draft parents, show inert labels for those parents or link to public summary pages only.
3. Privacy & tenancy suppression:

   * If any breadcrumb parent would reveal tenant-specific identifiers or PII, hide the parent or replace it with an innocuous label.
4. Low-confidence suppression:

   * If the inference service returns confidence < `0.8` and no editorial override is supplied, hide the breadcrumb partial rather than show a guess.
5. Minimal shell states:

   * Pages using a reduced shell (suppressed header/footer) must either provide a minimal escape link to a safe hub or omit breadcrumbs entirely. Legal/trust links must still be accessible per Appendix D.

---

## **10.6 API, Inference, Caching & Audit**

Integration and operational rules

1. APIs

   * Publish-time resolution API: `POST /api/breadcrumbs/publish-resolve` accepts `{ page_id }` and returns `{ parent_chain, chosen_parent, confidence }`. CMS must call this during publish and persist `chosen_breadcrumb_parent`.
   * Runtime read API (readonly): `GET /api/breadcrumbs/{page_id}` returns cached parent_chain for server render and for internal preview.
   * Inference API: `POST /api/inference/resolve-breadcrumb` used only by the publish pipeline when deterministic mapping is unavailable. Inference output must be persisted and not relied on at runtime.
2. Caching

   * Breadcrumb parent chains cached in CDN/edge for public pages. Cache invalidation triggered by page publish or owner/taxonomy changes.
3. Tracing & audit

   * Publish events must log: `{ page_id, chosen_breadcrumb_parent, resolution_method, inference_confidence, publish_timestamp, user_id }`.
   * Render and click events must emit analytics: `breadcrumb.render` and `breadcrumb.click` with `partial-id`, `page_id`, `position`, `user_role`.
4. Security

   * APIs must enforce permission checks for any tenant-scoped parent chains. Do not return tenant-only parents to anonymous callers.
5. Observability

   * Monitor inference confidence distribution and breadcrumb-render error rates. Alert if >1% of eligible pages render without a breadcrumb due to resolution failure.
...
1. Unit tests:
   ...
   * Breadcrumb resolution logic for owner/taxonomy/path fallbacks.
   * Inference decision gate (confidence threshold behavior).
2. Integration tests:

   * Publish pipeline triggers `publish-resolve` and persists `chosen_breadcrumb_parent`.
   * Page render includes BreadcrumbList JSON-LD matching visible trail.
3. Regression checks:

   * CI job compares rendered breadcrumb trail to Appendix B canonical parent chain for a random sample of pages.
4. Accessibility tests:

   * Automated axe checks for `<nav aria-label="Breadcrumb">`, focus order, and keyboard access to overflow menu.
5. Smoke checks:

   * Staging smoke test asserts no marketing discovery partials in suppressed shells and no breadcrumbs on checkout pages.

---

## **10.8 Implementation checklist (copy-paste)**

* CMS: require `page_id` and `owner_id` or `taxonomy_ids` for eligible page types.
* CMS: run `POST /api/breadcrumbs/publish-resolve` at publish and store `chosen_breadcrumb_parent`.
* Server render: read cached `parent_chain` from `/api/breadcrumbs/{page_id}` and embed JSON-LD and precomputed visible breadcrumb markup.
* Client: implement responsive overflow per 10.4 using the embedded chain.
* Analytics: emit `breadcrumb.render` and `breadcrumb.click` events.
* CI: add tests described in 10.7 and fail pipeline on mismatches.
* Monitoring: alert on publish-resolution failures and low inference-confidence rates.

---

# **11. Depth/Flatness Constraints**

This section controls how deep the IA may be and how flat it may become. Its purpose is to preserve discoverability, protect internal link equity, reduce editorial overhead, and keep product UIs usable. All rules below are normative: the CMS must enforce them and the CI audit must report violations.

## **11.0 Principles**

* Depth is for meaning; do not add levels for organisational vanity.
* Flatness is for speed; do not flatten to the point topics lose distinct identity.
* Marketing (public) surfaces, Documentation, and Product (Essentials) have different tolerances — treat each independently.
* All depth decisions use Appendix B (Page Inventory) as the canonical source of parent chains and Appendix E (Taxonomy) for category mapping.

---

## **11.1 Maximum Depth Thresholds (normative)**

Apply these maxima. Any page exceeding an absolute cap must be remediated before publish.

1. **Marketing / Public site (hubs, pages, guides)**

   * Normal maximum depth: **3** (Home › Hub › Page).
   * Absolute cap (exceptions only, documented): **4**.

2. **Documentation & API reference**

   * Normal maximum depth: **5** (Home › Docs › Area › Section › Leaf).
   * Absolute cap: **6** (very rare, justified and logged).

3. **Essentials / Product UI (dashboards, workflows)**

   * Normal maximum depth: **3** (Dashboard › Area › Task).
   * Absolute cap: **4** (admin/config screens only, justified).

4. **Taxonomy trees (category → subcategory → topic)**

   * Maximum depth: **3**.

5. **Sitemap / crawl depth**

   * Target crawlable depth ≤ **4** for marketing surfaces; deeper nodes must be surfaced from hub pages.

---

## **11.2 Minimum Depth Thresholds (normative)**

Prevent the IA collapsing into undifferentiated flatness.

1. **Marketing**

   * Minimum depth: **2** (Home › Topic). Campaign-only single-level sites are allowed but must be tagged `campaign=true` and isolated.

2. **Documentation**

   * Minimum depth: **2** (Docs › Area).

3. **Product**

   * Minimum depth: **1** (Dashboard). Features must be grouped into second-level areas where appropriate.

Rationale: at least one grouping level is required so owners and partials can aggregate and channel link equity.

---

## **11.3 Section-Specific Depth Limits and Behaviour (normative)**

Rules and actions applied by section.

1. **Hub / Topic clusters**

   * Preferred structure: Hub (level 1) › Major topic (level 2) › Article (level 3).
   * Articles at level 4 are allowed only for large reports; require owner sign-off and `depth_exception` metadata.

2. **Case studies / Resources**

   * Keep flat: place at level 2 or 3 and expose via hub lists and `related-stories` partials.

3. **Landing / Campaign pages**

   * Can be single-level. Mark `campaign=true` and exclude from permanent IA audits unless remapped.

4. **Search & tagging**

   * Discoverability via search/tags is orthogonal to hierarchy; do not use it to justify deeper structural depth.

5. **Product admin / configuration**

   * Deeper hierarchies (level 4 only) must include: stepper or clear breadcrumbs, search, and admin-specific navigation; require documented justification.

---

## **11.4 Enforcement, Audit and Remediation (operational)**

1. **CMS validation**

   * On save/publish compute page depth from `owner_id` → ancestors (Appendix B). Reject publish if page depth > absolute cap for its page type unless `depth_exception` present.

2. **Automated audit**

   * Weekly depth audit job that outputs: pages exceeding limits, orphan pages (no inbound internal links), clusters with poor owner link share. Send remediation tickets.

3. **Remediation patterns**

   * For deep pages: promote content to a higher level, create an intermediate hub, or merge into a sibling. Suggested action must be appended to the audit row.
   * For too-flat clusters: split owner content into subtopics or create intermediary hubs.

4. **Partial behaviour**

   * Discovery partials (`related-stories`, `for-your-industry`, `next-best-step`) must prefer owner-level links. Partials must not surface very deep leaves as primary suggestions without editorial justification.

5. **Breadcrumb interaction**

   * If a page exceeds normal visible depth, breadcrumb logic (§10) must collapse the trail and the audit must flag the deeper taxonomy for remediation.

6. **Exceptions**

   * Any exception requires `depth_exception` metadata with justification, approver, and expiry. Exceptions are visible in the audit and must be reviewed quarterly.

---

## **11.5 Tests, Metrics and Acceptance**

1. **CI tests**

   * Publish-time test: assert depth ≤ allowed or `depth_exception` present.
   * Regression: sample compare actual parent chains to Appendix B mappings.

2. **Monitoring metrics**

   * % pages exceeding allowed depth (target ≤ 0.5%).
   * % orphan pages (target ≤ 0.5%).
   * Owner link-share: % internal link weight to owner pages per cluster (target ≥ 60–70%).

3. **Acceptance**

   * Remediation complete when: no critical violations remain (tenant leaks, absolute cap breaches) and the audit metrics are within targets.

---

## **11.6 Change Control**

* Any increase to maximum depth for a section must be approved and recorded in Appendix B change log and the traceability table.
* Editorial or product sign-off required for reparenting proposals produced by the audit. Automated suggestion is permitted; automated structural change is forbidden.

---

## **11.7 Implementation checklist (copy-paste)**

* Add depth validation to CMS publish pipeline (compute owner → depth).
* Add weekly audit job to generate remediation CSV and pre-filled issue templates.
* Implement remediation workflow with editorial + product owners.
* Ensure partials default to owner-level preference and fail-safe hide behaviour for deep leaves.
* Add `depth_exception` metadata schema and enforcement rules.
* Wire monitoring alerts for depth metric thresholds.
...
This section is normative. Every permitted surface transition must be recorded in the repository-backed transition matrix and reference canonical `page_id` values from Appendix B. Transitions are enforced at publish time, at router/middleware, and in the API gateway. Any change to the matrix requires PR, traceable sign-off, and CI validation.

### 12.0 Core rules

* Every transition is a first-class object with the schema described below. Only transitions present in `transition-matrix.json` are allowed.
* Transition types: `deterministic` (allowed when preconditions are satisfied) and `conditional` (requires runtime checks). Inference may supply *recommendations* but must not execute any transition that affects tenancy, billing, or evidence without explicit user action and server-side validation.
* Enforcement layers: (1) CMS/publish-time validation; (2) router/middleware (runtime gate); (3) server transition API (`/api/transition/intent`); (4) API gateway/OPA policies for permission checks.
* Auditing: every transition attempt and completion must be logged with correlation id.
...
Each transition object in `transition-matrix.json` must include:

* `id` — unique transition id.
* `from` — canonical `page_id`.
* `to` — canonical `page_id` or route template.
* `type` — `deterministic` | `conditional`.
* `allowed_runtimes` — list e.g. `["marketing","public","essentials"]`.
* `preconditions` — object of required checks (see 12.3).
* `required_permissions` — optional list e.g. `["owner","editor"]`.
* `preserve` — list of keys to persist across transition (only short-lived tokens / ids).
* `entry_handler` / `exit_handler` — server-side handlers to run.
* `audit_level` — `low` | `medium` | `high` | `critical`.
* `notes` — editorial/operational notes (required for conditional transitions).

#### Representation & storage

* Store `transition-matrix.json` in the repo root `/config/transition-matrix.json`. CI validates schema and that every `from` and `to` exists in Appendix B.

#### Examples (canonical)

* Marketing article → Feature detail

  ```
  { "id":"t_article_to_feature_v1",
    "from":"page:guides/secure-deploy",
    "to":"page:features/secure-deploy",
    "type":"deterministic",
    "allowed_runtimes":["marketing"],
    "preconditions":{},
    "preserve":["campaign_id","referrer_id"],
    "audit_level":"low"
  }
  ```
* Signup → Onboard (requires auth)

  ```
  { "id":"t_signup_to_onboard_v1",
    "from":"page:signup",
    "to":"page:app/onboard",
    "type":"deterministic",
    "allowed_runtimes":["marketing"],
    "preconditions":{"auth_required":true,"email_verified":true},
    "preserve":["signup_session_id"],
    "audit_level":"medium"
  }
  ```
...
Explicit prohibitions (server must deny):

* Marketing → tenant admin pages (`/admin/*`) that reveal tenant identifiers.
* Any direct route to payment capture endpoints (e.g., `/billing/charge`) without explicit user confirmation + server-side signed payment intent.
* Essentials evidence-edit → public report preview unless `publish_request` is completed and tenant consent recorded.
* Transitions that would inject tenant identifiers or PII into marketing URLs (strip or sanitize before transition).
* Any transition triggered by inference without user action that results in billing, tenant exposure, or evidence publication.

Enforcement:

* Router/middleware must return `403` / `redirect` for forbidden attempts. CI asserts forbidden route pairs are present in matrix test fixtures.

---

### **12.3 Conditioned Transitions**

#### Standard preconditions (canonical set)

* `auth_required` — boolean.
* `permissions_required` — array of roles.
* `subscription_tier` — one or more allowed tiers.
* `data_presence` — object with required server-side keys (e.g., `{"evidence_complete":true}`).
* `feature_flag` — runtime feature gating.
* `consent_recorded` — data publication consents present.
* `no_active_transaction` — prevents concurrent payment flows.

#### State preservation rules

* Preserve only minimal, audited keys: `campaign_id`, `referrer_id`, `draft_id`, `requested_asset_id`. Never pass PII or tenant IDs in query strings. For complex state, persist server-side and pass a short-lived `draft_id` or `transition_token`.

#### Secure transition flow (must be implemented)

1. Client calls `POST /api/transition/intent` with `{ from, to, payload }`.
2. Server evaluates matrix entry, checks preconditions and permissions, and returns an action:

   * `allowed` + server-signed `transition_token` + `target_url`, or
   * `redirect` (to login or consent), or
   * `deny` with reason.
3. Client navigates to `target_url?tt=<token>` or completes server redirect. Server validates `tt` and executes `entry_handler`. Tokens are short-lived and single-use.
4. All steps emit telemetry: `transition.attempt`, `transition.allowed/denied`, `transition.complete`.

#### Use of inference

* Inference returns `recommended_transitions` with confidence scores. UI may display recommendations via marketing partials only. Recommendations cannot bypass `POST /api/transition/intent` checks.
...
* `success` — completed. May issue server ticket (invoice_id, report_id).
* `cancelled_by_user` — user aborted; record `cancellation_reason`.
* `failed_recoverable` — transient error; provide retry route.
* `failed_unrecoverable` — requires manual intervention; escalate to support.
* `requires_action` — intermediate terminal (e.g., pending consent); includes expiry.
* `archived` — content or journey archived; route to archival notice.

Recording:

* At terminal point write trace: `{ journey_id, page_id, terminal_state, user_id?, correlation_id, timestamp, reason }`. Retain per Appendix C retention policy.

---

### Enforcement, testing & observability

#### CI & validation

* Validate `transition-matrix.json` schema and that `from`/`to` exist in Appendix B.
* Publish-time: CMS must block CTAs or partial placements that reference transitions not declared in the matrix.

#### Runtime checks

* Router/middleware validates `transition_token` and enforces OPA/Rego policies for permission/consent.
* API gateway denies transitions that expose tenant IDs in marketing runtime.
* All transitions generate audit logs and metrics.

#### Tests

* Unit: matrix parsing, precondition checks, token issuance/validation.
* Integration: intent flow `POST /api/transition/intent` → token issuance → server redirect → entry handler.
* Security: ensure no PII leakage in redirect URLs and that forbidden transitions are rejected.

#### Telemetry & alerts

* Emit `transition.*` events with correlation id.
* Dashboards: denied transitions, high-frequency denied attempts, terminal failure rates. Alert when denied attempts spike or terminal failure rate crosses thresholds.

---

### Implementation checklist (copy-paste)

1. Add `/config/transition-matrix.json` and enforce schema in CI.
2. Require CMS to select or validate `matrix_transition_id` when authors add CTAs or partials that imply a cross-surface hop.
3. Implement `POST /api/transition/intent` with server-side evaluation and short-lived `transition_token`.
4. Implement router/middleware validation for `transition_token` and OPA policy checks.
5. Add audit logging for all transition lifecycle events.
6. Add tests: unit, integration, security.
7. Add dashboards and alerts for denied transitions and terminal-state failures.
8. Quarterly review of conditioned transitions and inference recommendations; update matrix and re-run CI.

---

### Minimal example transition object (copyable)

```json
{
  "id": "t_signup_to_onboard_v1",
  "from": "page:features/secure-deploy",
  "to": "page:app/onboard",
  "type": "deterministic",
  "allowed_runtimes": ["marketing"],
  "preconditions": {"auth_required": true, "email_verified": true},
  ...
  "preserve": ["signup_session_id"],
  "entry_handler": "log_signup_intent",
  "audit_level": "medium",
  "notes": "Signup → onboard requires verified email and server-signed transition token"
}
```
